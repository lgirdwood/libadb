...
static inline void equ_to_kd_vertex(double ra, double dec,
                                    struct kd_vertex *v) {
...
static struct kd_base_elem *kd_x_select_elem(struct kd_base *mbase, int x_start,
  ...
  if (x_id == x_end)
    child =
        kd_y_select_elem(mbase, x_id, x_end, y_start, y_end, z_start, z_end);
  else
    child = kd_y_select_elem(mbase, x_id + 1, x_end, y_start, y_end, z_start,
                             z_end);
  if (child) {
    child->kd.parent = parent->bid;
...
static struct kd_base_elem *kd_y_select_elem(struct kd_base *mbase, int x_start,
  ...
  if (y_id == y_end)
    child =
        kd_z_select_elem(mbase, x_start, x_end, y_id, y_end, z_start, z_end);
  else
    child = kd_z_select_elem(mbase, x_start, x_end, y_id + 1, y_end, z_start,
                             z_end);
  if (child) {
    child->kd.parent = parent->bid;
    parent->kd.child[1] = child->bid;
...
static struct kd_base_elem *kd_z_select_elem(struct kd_base *mbase, int x_start,
  ...
  if (z_id == z_end)
    child =
        kd_x_select_elem(mbase, x_start, x_end, y_start, y_end, z_id, z_end);
  else
    child = kd_x_select_elem(mbase, x_start, x_end, y_start, y_end, z_id + 1,
                             z_end);
  if (child) {
    child->kd.parent = parent->bid;
    parent->kd.child[1] = child->bid;
...
static void insert_elem_object(struct kd_init *init,
...
int import_build_kdtree(struct adb_db *db, struct adb_table *table) {
  ...
  adb_info(db, ADB_LOG_CDS_KDTREE, "Preparing KD Tree for %s with %d objects\n",
  ...
  qsort(mbase.x_base, table->object.count, sizeof(struct kd_elem), elem_x_cmp);
  qsort(mbase.y_base, table->object.count, sizeof(struct kd_elem), elem_y_cmp);
  qsort(mbase.z_base, table->object.count, sizeof(struct kd_elem), elem_z_cmp);
