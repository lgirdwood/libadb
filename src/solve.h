...
static void debug_init(struct adb_object_set *set) {
  ...
  for (i = 0; i < adb_size(dnames); i++) {
    found = adb_set_get_object(set, dnames[i], ADB_FIELD_DESIGNATION, &dobj[i]);
    if (found <= 0)
  ...
  for (i = 0; i < adb_size(snames); i++) {
    found = adb_set_get_object(set, snames[i], ADB_FIELD_DESIGNATION, &sobj[i]);
    if (found <= 0)
      fprintf(stderr, "can't find %s\n", snames[i]);
    else
  ...
  do {                                                                         \
    if (stage < runtime->debug)                                                \
      fprintf(stdout,                                                          \
              ...
              "%f tests %d no %d\n",                                           \
              stage, object1->designation, object1->mag, object2->designation, \
              object2->mag, dist, min, max, num, i);                           \
  } while (0)
  ...
  do {                                                                         \
    if (runtime->debug)                                                        \
      fprintf(stdout, "%d: object %s (%3.3f) dist %f min %f max %f\n", num,    \
              object->designation, object->mag, dist, min, max);               \
  } while (0)
