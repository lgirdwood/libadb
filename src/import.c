...
static int float_alt_import(struct adb_object *object, int offset, char *src,
                            char *src2) {
  char *ptr, *dest = (char *)object + offset;
...
static int double_alt_import(struct adb_object *object, int offset, char *src,
                             char *src2) {
  char *ptr, *dest = (char *)object + offset;
...
static void get_import_buffer_size(struct adb_db *db, struct adb_table *table) {
  int i;
...
static void histo_depth_calc(struct adb_db *db, struct adb_table *table,
  ...
  for (j = db->htm->depth; j >= 0; j--) {
    ...
    table->depth_map[j].max_value = table->object.min_value + old_start * histo;
    old_start = start;
...
static int table_histogram_import(struct adb_db *db, struct adb_table *table,
                                  FILE *f) {
...
static int table_histogram_alt_import(struct adb_db *db,
  ...
  for (j = 0; j < table->object.count || table->object.count == 0; j++) {
    ...
    strncpy(buf, line + key->key_field.text_offset, key->key_field.text_size);
    strncpy(buf2, line + key->alt_field.text_offset, key->alt_field.text_size);
...
static int import_object_ascending(struct adb_db *db, struct adb_object *object,
  ...
  if (!trixel) {
    adb_error(db, "no trixel at RA %f DEC %f\n", adb_object_ra(object) * R2D,
              adb_object_dec(object) * R2D);
    return -EINVAL;
  }
...
static int import_rows(struct adb_db *db, int table_id, FILE *f) {
  ...
  for (j = 0; j < table->object.count; j++) {
    ...
    if (warn) {
      adb_vdebug(db, ADB_LOG_CDS_IMPORT, "At line %d with length %d :-\n",
...
out:
  adb_info(db, ADB_LOG_CDS_IMPORT,
           "Got %d short, %d blank records %d warnings\n", short_records, blank,
           warn);
...
static int import_rows_with_alternatives(struct adb_db *db, int table_id,
                                         FILE *f) {
  struct adb_table *table;
  ...
  for (j = 0; j < table->object.count; j++) {
    ...
    for (k = 0; k < table->object.num_alt_fields; k++) {
      ...
      import = table->import.alt_field[k].import(
          object, table->import.alt_field[k].key_field.struct_offset, buf,
          buf2);
      if (import < 0) {
...
int adb_table_import_new(struct adb_db *db, const char *cat_class,
  ...
  if (table->cds.index == NULL)
    goto err;
  sprintf(remote, "%s%s%s%s%s%s", db->lib->remote, "/", cat_class, "/", cat_id,
          "/");
  ...
  for (files = 0; files < readme->num_files; files++) {
    ...
    adb_debug(db, ADB_LOG_CDS_TABLE, "table %s filename %s num desc %d\n",
    ...
    if (file_info->num_desc &&
      ...
      adb_info(db, ADB_LOG_CDS_TABLE, "  creating table: %s\n",
               file_info->name);
      return table_id;
    }
  }
...
int adb_table_import(struct adb_db *db, int table_id) {
  ...
  adb_warn(db, ADB_LOG_CDS_TABLE, "Error failed to FTP CDS data files for %s\n",
           table->path.file);
...
import:
	...
	for (i = 0;
i < adb_size(file_extensions); i++) {
  memset(file, 0, ADB_PATH_SIZE);
  ...
  if (ret < 0) {
    adb_warn(db, ADB_LOG_CDS_TABLE, "Error failed to import CDS table %s %d\n",
             table->path.file, ret);
  } else
    goto schema;
}
