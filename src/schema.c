...
static void schema_dump(struct adb_db *db, struct adb_table *table) {
  ...
  for (i = 0; i < table->object.field_count; i++) {
    adb_info(
        db, ADB_LOG_CDS_SCHEMA, "%d\t%s\t%d\t%d\t%d\t%d\t%d\t%s\n", i,
        table->import.field[i].symbol, table->import.field[i].struct_offset,
        table->import.field[i].struct_bytes, table->import.field[i].text_offset,
